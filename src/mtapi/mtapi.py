...
from typing import Any, Literal, NamedTuple, Optional, TypeAlias, TypedDict
...
from mtapi._mtapithreader import MtapiThreader

logger = logging.getLogger(__name__)


# List of pair: Lat, Lng
Location: TypeAlias = list[float]
...
class Station:
    ...
    def __init__(self, d: StationDict):
        ...
        self.last_update: Optional[Any]

    def add_train(
        self, route_id: str, direction: Literal["N", "S"], train_time, feed_time
    ):
        print(type(train_time), type(feed_time))
        self.routes.add(route_id)
        self.trains[direction].append(Train(route_id, train_time))
...
class Mtapi(object):
    ...
    def _load_mta_feed(self, feed_url: str):
        try:
    ...
    def get_by_point(self, point: tuple[float, float], limit=5):
        if self.is_expired():
            self.update()
